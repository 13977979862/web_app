    ...
    <div class="bet-box">
      ...
      <div class="bet-box-fr wrapper">
        <div class="content">
          ...
          >
            <div
              v-show="!(item.menuid == 967 && item2.title == '第三球' &&  zhi_xuan_id != 7066)"
            ...
            >
              ...
              <div class="flex-bodys" v-for="(item3,index3) in item2.data" :key="index3">
                ...
                >
                  ...
                  <span
                    class="_it2"
                    v-show="(item.menuid != 964 &&  item.menuid != 967) || item2.title == '选择类型'"
                  >{{item4.odds}}</span>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    _checklist(e, menuListItem, currentId, numberArrayList, targetItem) {
      ...
      // }
      //选择已选中的玩法
      if (menuListItem.menuid != 964 && menuListItem.menuid != 967) {
        if (e.currentTarget.className.indexOf("flex-html bet-choose") == -1) {
      ...
      if (menuListItem.menuid == 964) {
        ...
        if (numberArrayList.title == "选择号码") {
          ...
          if (this.lian_ma_id.length == 0) {
            ...
            (this.lian_ma_id[0] == 7059 ||
              this.lian_ma_id[0] == 7063 ||
              this.lian_ma_id[0] == 7064) &&
        ...
        if (this.lian_ma_id[0] == 7063) {
        ...
        } else if (this.lian_ma_id[0] == 7064) {
          ...
          if (
            (this.lian_ma_id[0] == 7056 && this.lian_ma_number.length < 2) ||
            (this.lian_ma_id[0] == 7057 && this.lian_ma_number.length < 3) ||
            (this.lian_ma_id[0] == 7058 && this.lian_ma_number.length < 4) ||
            ((this.lian_ma_id[0] == 7059 ||
              this.lian_ma_id[0] == 7063 ||
              this.lian_ma_id[0] == 7064) &&
              this.lian_ma_number.length < 5) ||
            (this.lian_ma_id[0] == 7060 && this.lian_ma_number.length < 6) ||
            (this.lian_ma_id[0] == 7061 && this.lian_ma_number.length < 7) ||
            (this.lian_ma_id[0] == 7062 && this.lian_ma_number.length < 8)
          ) {
            this.lianmaSum = 0;
          } else {
            this.lianmaSum = 1;
      ...
      if (menuListItem.menuid == 967) {
        ...
        //吧所有直选中的选择类型ID放到一个数组中。连码的id是967，id如果更改，需要更新代码
    ...
    matchingNumber(firstNumber, secondNumber, thirdNumber) {
      ...
      } else if (this.zhi_xuan_id[0] === 7066) {
        if (
    ...
    _validateLianMa(lianmaDom) {
      ...
      if (typeId == "7056" && lianmaDom.length < 3) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
      if (typeId == "7057" && lianmaDom.length < 4) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
      if (typeId == "7058" && lianmaDom.length < 5) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
      if (typeId == "7059" && lianmaDom.length < 6) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
      if (typeId == "7063" && lianmaDom.length < 3) {
        this.infotips("至少选择两个号码!"); //信息提示弹框
        return true;
      }
      if (typeId == "7064" && lianmaDom.length < 4) {
        this.infotips("至少选择三个号码!"); //信息提示弹框
        return true;
      }
      if (typeId == "7060" && lianmaDom.length < 7) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
      if (typeId == "7061" && lianmaDom.length < 8) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }

      if (typeId == "7062" && lianmaDom.length < 9) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
