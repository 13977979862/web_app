    ...
    <div class="bet-box">
      ...
      <div class="bet-box-fr wrapper">
        <div class="content">
          ...
          >
            <div
              v-show="!(item.menuid == 940 && item2.title == '第三球' &&  zhi_xuan_id != 6897)"
            ...
            >
              ...
              <div class="flex-bodys" v-for="(item3,index3) in item2.data" :key="index3">
                ...
                >
                  ...
                  <span
                    class="_it2"
                    v-show="(item.menuid != 937 &&  item.menuid != 940) || item2.title == '选择类型'"
                  >{{item4.odds}}</span>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    _checklist(e, menuListItem, currentId, numberArrayList, targetItem) {
      ...
      // }
      //选择已选中的玩法
      if (menuListItem.menuid != 937 && menuListItem.menuid != 940) {
        if (e.currentTarget.className.indexOf("flex-html bet-choose") == -1) {
      ...
      //let idList = [];
      let lian_ma_id = null;
      console.log(menuListItem.menuid);
      if (menuListItem.menuid == 937) {
        ...
        if (numberArrayList.title == "选择号码") {
          ...
          if (this.lian_ma_id.length == 0) {
            ...
            (this.lian_ma_id[0] == 6890 ||
              this.lian_ma_id[0] == 6894 ||
              this.lian_ma_id[0] == 6895) &&
        ...
        if (this.lian_ma_id[0] == 6894) {
        ...
        } else if (this.lian_ma_id[0] == 6895) {
          ...
          if (
            (this.lian_ma_id[0] == 6887 && this.lian_ma_number.length < 2) ||
            (this.lian_ma_id[0] == 6888 && this.lian_ma_number.length < 3) ||
            (this.lian_ma_id[0] == 6889 && this.lian_ma_number.length < 4) ||
            ((this.lian_ma_id[0] == 6890 ||
              this.lian_ma_id[0] == 6894 ||
              this.lian_ma_id[0] == 6895) &&
              this.lian_ma_number.length < 5) ||
            (this.lian_ma_id[0] == 6891 && this.lian_ma_number.length < 6) ||
            (this.lian_ma_id[0] == 6892 && this.lian_ma_number.length < 7) ||
            (this.lian_ma_id[0] == 6893 && this.lian_ma_number.length < 8)
          ) {
            this.lianmaSum = 0;
          } else {
            this.lianmaSum = 1;
      ...
      if (menuListItem.menuid == 940) {
        ...
        //吧所有直选中的选择类型ID放到一个数组中。连码的id是940，id如果更改，需要更新代码
    ...
    matchingNumber(firstNumber, secondNumber, thirdNumber) {
      ...
      } else if (this.zhi_xuan_id[0] === 6897) {
        if (
    ...
    _validateLianMa(lianmaDom) {
      ...
      if (typeId == "6887" && lianmaDom.length < 3) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }

      if (typeId == "6888" && lianmaDom.length < 4) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }

      if (typeId == "6889" && lianmaDom.length < 5) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }

      if (typeId == "6890" && lianmaDom.length < 6) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }

      if (typeId == "6894" && lianmaDom.length < 3) {
        this.infotips("至少选择两个号码!"); //信息提示弹框
        return true;
      }

      if (typeId == "6895" && lianmaDom.length < 4) {
        this.infotips("至少选择三个号码!"); //信息提示弹框
        return true;
      }

      if (typeId == "6891" && lianmaDom.length < 7) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }

      if (typeId == "6892" && lianmaDom.length < 8) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }

      if (typeId == "6893" && lianmaDom.length < 9) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
