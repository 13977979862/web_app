    ...
    <div class="bet-box">
      ...
      <div class="bet-box-fr wrapper">
        <div class="content">
          ...
          >
            <div
              v-show="!(item.menuid == 913 && item2.title == '第三球' &&  zhi_xuan_id != 6728)"
            ...
            >
              ...
              <div class="flex-bodys" v-for="(item3,index3) in item2.data" :key="index3">
                ...
                >
                  ...
                  <span
                    class="_it2"
                    v-show="(item.menuid != 910 &&  item.menuid != 913) || item2.title == '选择类型'"
                  >{{item4.odds}}</span>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    _checklist(e, menuListItem, currentId, numberArrayList, targetItem) {
      ...
      // }
      //选择已选中的玩法
      if (menuListItem.menuid != 910 && menuListItem.menuid != 913) {
        if (e.currentTarget.className.indexOf("flex-html bet-choose") == -1) {
      ...
      if (menuListItem.menuid == 910) {
        ...
        if (numberArrayList.title == "选择号码") {
          ...
          if (this.lian_ma_id.length == 0) {
            ...
            (this.lian_ma_id[0] == 6721 ||
              this.lian_ma_id[0] == 6725 ||
              this.lian_ma_id[0] == 6726) &&
        ...
        if (this.lian_ma_id[0] == 6725) {
        ...
        } else if (this.lian_ma_id[0] == 6726) {
          ...
          if (
            (this.lian_ma_id[0] == 6718 && this.lian_ma_number.length < 2) ||
            (this.lian_ma_id[0] == 6719 && this.lian_ma_number.length < 3) ||
            (this.lian_ma_id[0] == 6720 && this.lian_ma_number.length < 4) ||
            ((this.lian_ma_id[0] == 6721 ||
              this.lian_ma_id[0] == 6725 ||
              this.lian_ma_id[0] == 6726) &&
              this.lian_ma_number.length < 5) ||
            (this.lian_ma_id[0] == 6722 && this.lian_ma_number.length < 6) ||
            (this.lian_ma_id[0] == 6723 && this.lian_ma_number.length < 7) ||
            (this.lian_ma_id[0] == 6724 && this.lian_ma_number.length < 8)
          ) {
            this.lianmaSum = 0;
          } else {
            this.lianmaSum = 1;
    ...
    matchingNumber(firstNumber, secondNumber, thirdNumber) {
      ...
      } else if (this.zhi_xuan_id[0] === 6728) {
        if (
    ...
    _validateLianMa(lianmaDom) {
      ...
      if (typeId == "6718" && lianmaDom.length < 3) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
      if (typeId == "6719" && lianmaDom.length < 4) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
      if (typeId == "6720" && lianmaDom.length < 5) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
      if (typeId == "6721" && lianmaDom.length < 6) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
      if (typeId == "6725" && lianmaDom.length < 3) {
        this.infotips("至少选择两个号码!"); //信息提示弹框
        return true;
      }
      if (typeId == "6726" && lianmaDom.length < 4) {
        this.infotips("至少选择三个号码!"); //信息提示弹框
        return true;
      }
      if (typeId == "6722" && lianmaDom.length < 7) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
      if (typeId == "6723" && lianmaDom.length < 8) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }

      if (typeId == "6724" && lianmaDom.length < 9) {
        this.infotips("请正确选择玩法!"); //信息提示弹框
        return true;
      }
