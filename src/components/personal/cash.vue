<template>
    <div class="cash">
        <header class="top">
            <div class="inner">
                <span class="iconfont icon-fanhui cw" @click="_personalcenter"></span>
                <p class="title" id="title">提款记录</p>
            </div>
        </header>
        <!---->
                        ...
                        <table class="f_table" cellpadding="0" cellspacing="1">
                            ...
                            <tbody>
                                <tr v-for="(item,index) in all_Data_list">
                                    <td v-html="_changbr(item.account)"></td>
                                    <td>{{item.amount}}</td>
                                    <td>{{item.status}}</td>
                                    <td>{{item.info}}</td>
                                    <td v-html="_time(item.actionTime)"></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            _getAjaxdata () {
                ...
                $api.$post('Api/User/cash', _data).then((res) => {
